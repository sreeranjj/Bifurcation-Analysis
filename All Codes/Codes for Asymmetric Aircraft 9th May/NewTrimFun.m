...
mat=[ I , B ;
  ...
  %CD=CD0 + CD_de*x(4); 
  CD= interp1(ALPHA_BREAK,cd0,x(1),'spline') + interp1(ALPHA_BREAK,cd_de,x(1),'spline')*x(4);
  %CL=CL0 +CL_de*x(4);
  CL=interp1(ALPHA_BREAK,clift,x(1),'spline') +interp1(ALPHA_BREAK,clift_de,x(1),'spline')*x(4);
  CT=x(3)*Tm/Q*S;
