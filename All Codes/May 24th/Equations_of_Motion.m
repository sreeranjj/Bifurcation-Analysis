...
f1=-q*w + r*v +  (q^2+r^2)*xcm -p*q*ycm - r*p*zcm -g*sin(theta) +(1/m_tot)*(X);
f2=-r*u + p*w -p*q*xcm + (r^2+p^2)*ycm - q*r*zcm +g*cos(theta)*sin(phi) +(1/m_tot)*(Y); 
f3=-p*v + q*u -r*p*xcm - q*r*ycm + (p^2+q^2)*zcm +g*cos(theta)*cos(phi) +(1/m_tot)*(Z) ; 

f4=(1/Ixx)*(m_tot*zcm*f2 - m_tot*ycm*f3 + Ixz*p*q + (Iyy-Izz)*q*r - Ixy*r*p + Iyz*(q^2-r^2)+ m*(q*u-p*v)*ycm + m*(r*u-p*w)*zcm -zcm*m*g*cos(theta)*sin(phi) + ycm*m*g*cos(theta)*cos(phi)+Q*S*b*Cl);

f5=(1/Iyy)*(m_tot*xcm*f3 - m_tot*zcm*f1 - Iyz*p*q + Ixy*q*r + (Izz-Ixx)*r*p + Ixz*(r^2-p^2)+m*(p*v-q*u)*xcm + m*(r*v-q*w)*zcm -zcm*m*g*sin(theta) - xcm*m*g*cos(theta)*cos(phi) + Q*S*cbar*Cm );

f6=(1/Izz)*(m_tot*ycm*f1 - m_tot*xcm*f2 + (Ixx-Iyy)*p*q - Ixz*q*r + Iyz*r*p + Ixy*(p^2-q^2)+m*(p*w-r*u)*xcm + m*(q*w-r*v)*ycm +ycm*m*g*sin(theta) - xcm*m*g*cos(theta)*sin(phi) +Q*S*b*Cn);
